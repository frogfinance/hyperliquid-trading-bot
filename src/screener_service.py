...
class Asset:
    def __init__(self, id : int, rawAssetName : str, tf : str, hyperliquidBot : HyperLiquidExecutionService, is_longStrat : bool ):
        self.id = id
    ...
    def change_txn_amount(self, amount):
        logging.info(f"Changing Ape Amount for {self.rawAssetName} to {amount}")
    ...
    def get_slPrice(self) -> float:
        ...
        history = self.exchange.fetch_ohlcv(self.coinpair, timeframe=self.tf, limit=limit)
    ...
    def changehma(self, length : int):
        ...
        self.hmaTrend = None
        self.generateSignal()
        logging.info(f"{self.coinpair} hma changed to {length}")        
    ...
    def generateSignal(self):
        ...
        logging.info(f"{self.coinpair} Last: {last} Second Last: {secondLast}")
                        
        if self.hmaTrend == None:
            logging.info(f"{self.coinpair} Initialising HMA Trend as {last > secondLast}")
        ...
        if buycondition:
            logging.info(f"{self.coinpair} Buy Signal on {self.tf} timeframe")
            self.webhook.send(f"@everyone {self.coinpair} Buy Signal on {self.tf} timeframe")
            self.hmaTrend = True
            
            if self.is_longStrat:
                with lock:                    
                    res = self.hyperLiquidBot.generate_order(self.rawAssetName, self.txn_USDTAmount, True, self.setSl, lowest)
        ...
        if sellcondition:
            logging.info(f"{self.coinpair} Sell Signal on {self.tf} timeframe")
            self.webhook.send(f"@everyone {self.coinpair} Sell Signal on {self.tf} timeframe")
            self.hmaTrend = False
            ...
            if not self.is_longStrat:
                with lock:
                    res = self.hyperLiquidBot.generate_order(self.rawAssetName, self.txn_USDTAmount, False, self.setSl, highest)
    ...
    # def run(self):
    #     # # MUST REMOVE THIS
    #     # logging.info(f"{self.coinpair} {self.tf} Running. Sleeping")
    #     # time.sleep(100)
    #     # logging.info(f"{self.coinpair} {self.tf} Waking up")
    ...
    #         logging.info(f"{self.coinpair} {self.tf} Sleeping for {timeToSleep}")
    ...
    def run(self):
        ...
        while True:
            ...
            if condition:
                logging.info(f"{self.coinpair} {self.tf} running inside condition.")
                self.get_historical_data()
                self.generateSignal()
            ...
            if diff_seconds < 30:
                if diff_seconds > 0:
                    logging.info(f"{self.coinpair} {self.tf} Sleeping for {diff}")
                    time.sleep(diff_seconds)
                else:
                    logging.info(f"{self.coinpair} {self.tf} Sleeping for 0.1 second")
                    time.sleep(0.1)
            else:
                logging.info(f"{self.coinpair} {self.tf} Sleeping for 20 seconds")
                time.sleep(20)                
...
class Screener:
    def __init__(self):
        self.assets =  []
        self.idcount = 0
        self.hyperliquidBot : Optional[HyperLiquidExecutionService] = None 
        logging.info("Screener loaded")
        
    def addAsset(self, asset : str, tf : str , sl : int, hma : int, size : float, leverage : int, is_long : bool):
    ...
    def removeAsset(self, id):
        for a in self.assets:
            ...
            if a.id == id:
                ...
                logging.info(f"Removed {a.coinpair}")
                return
